...
namespace RootMotion.FinalIK
{
    ...
    public partial class IKSolverVR : IKSolver
    {
        ...
        public class Arm : BodyPart
        {
            ...
            private Vector3 smallMargin = new Vector3(0.0001f, 0f, 0f);
            ...
            {
                get { return bones[hasShoulder ? 1 : 0]; }
            }

            private VirtualBone forearm
            {
                get { return bones[hasShoulder ? 2 : 1]; }
            }

            private VirtualBone hand
            {
                get { return bones[hasShoulder ? 3 : 2]; }
            }
                ...
                bool hasShoulders, bool hasToes, bool hasLegs, int rootIndex, int index)
            {
                ...
                if (!initiated)
                {
                    ...
                    upperArmBendAxis =
                        Vector3.Cross(Quaternion.Inverse(upperArmRotation) * (forearmPosition - upperArmPosition),
                            upperArmForwardAxis);
                    if (upperArmBendAxis == Vector3.zero)
                    {
                        Debug.Log(
                            "VRIK can not calculate which way to bend the arms because the arms are perfectly straight. Please rotate the elbow bones slightly in their natural bending direction in the Editor.");
                    }
                }
            ...
            public void Solve(bool isLeft)
            {
                ...
                if (hasShoulder && shoulderRotationWeight > 0f && LOD < 1)
                {
                    switch (shoulderRotationMode)
                    {
                        case ShoulderRotationMode.YawPitch:
                            ...
                            workingSpace =
                                Quaternion.AngleAxis(isLeft ? shoulderPitchOffset : -shoulderPitchOffset,
                                    chestForward) * workingSpace;
                            ...
                            float p = Mathf.Clamp(
                                pitch * positionWeight * shoulderRotationWeight * shoulderTwistWeight * 2f, 0f, 180f);
                        ...
                        case ShoulderRotationMode.FromTo:
                            ...
                            Quaternion r = Quaternion.FromToRotation(
                            ...
                            pitchAngle =
                                Mathf.Clamp(
                                    pitchAngle * shoulderRotationWeight * shoulderTwistWeight * 2f * positionWeight, 0f,
                                    180f);

                            shoulder.solverRotation = Quaternion.AngleAxis(pitchAngle,
                                                          shoulder.solverRotation *
                                                          (isLeft ? shoulder.axis : -shoulder.axis)) *
                                                      shoulder.solverRotation;
                            upperArm.solverRotation = Quaternion.AngleAxis(pitchAngle,
                                                          upperArm.solverRotation *
                                                          (isLeft ? upperArm.axis : -upperArm.axis)) *
                                                      upperArm.solverRotation;
                            break;
                    }
                }
                ...
                if (LOD < 1 && positionWeight > 0f)
                {
                    ...
                    Quaternion space = Quaternion.LookRotation(
                        (upperArm.solverRotation * upperArmBendAxis) + smallMargin,
                        forearm.solverPosition - upperArm.solverPosition);
                    ...
                    upperArm.solverRotation =
                        Quaternion.AngleAxis(angle * positionWeight, forearm.solverPosition - upperArm.solverPosition) *
                        upperArm.solverRotation;

                    // Fix forearm twist relative to upper arm
                    Quaternion forearmFixed = upperArm.solverRotation * forearmRelToUpperArm;
                    Quaternion fromTo = Quaternion.FromToRotation(forearmFixed * forearm.axis,
